...
<div class="wrap" id="of_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">Opções atualizadas</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">Opções resetadas</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2><?php echo $themename; ?> <?php //echo get_option('blogname'); ?></h2>
      </div>
    ...
    <div id="main">
      <div id="of-nav">
        <ul>
         ...
         <div class="logo" style="background-color:#BBE1EC; text-align:center; height:150px;">
		    <?php $logo_url  = get_option('ezine_logo'); ?>
		    <a href="<?php bloginfo('url');?>"><img src="<?php if ($logo_url != "") {echo $logo_url;} else { echo bloginfo('template_directory').'/images/logo.png';}?>" style="margin-top:50px" alt="<?php bloginfo('blogname');?>" /></a>
          </div>
      </div>
    ...
    <input type="submit" value="Salvar opções" class="button-primary" />
  </form>
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    <span class="submit-footer-reset">
    <input name="reset" type="submit" value="Resetar opções" class="button submit-button reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
...
function of_style_only() {
	...
	wp_enqueue_style('facebox.css', OF_DIRECTORY.'/admin/facebox/facebox.css');        
}	
...
function of_load_only() {
	...
	wp_enqueue_script('facebox', OF_DIRECTORY.'/admin/facebox/facebox.js', array('jquery'));
	wp_enqueue_script('jquery', OF_DIRECTORY.'/admin/facebox/jquery.js', array('jquery'));        
...
<script type="text/javascript" language="javascript">
    ...
    jQuery(document).ready(function($) {
      $('a[rel*=facebox]').facebox({
        loading_image : '<?php echo bloginfo('template_directory').'/facebox/loading.gif' ?>',
        close_image   : '<?php echo bloginfo('template_directory').'/facebox/fechar.gif' ?>'
      })
    })
...
<script type="text/javascript">
			...
			new AjaxUpload(clickedID, {
				  ...
				  onComplete: function(file, response) {
					...
					clickedObject.text('Upload Imagem');	
